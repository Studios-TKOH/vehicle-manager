  ...
  return (
    <div className={styles.container}>
        ...
        <div>
          ...
          <div className="flex gap-2">
            <input
              ...
              readOnly
              className={`${styles.formInput} w-1/3 text-center bg-slate-100 font-bold`}
            />
            <input
              type="text"
              value={documento.correlativo}
              readOnly
              className={`${styles.formInput} w-2/3 bg-slate-100`}
            />
          </div>
        </div>
        <div>
      ...
      <div className={styles.actionRow}>
        ...
        >
          {inlineInputs.placa ? (
            <input
              ...
              className="bg-transparent border-white/50 border-b outline-none w-full text-center"
        ...
        {/* NUEVOS BOTONES DE KILOMETRAJE */}
        <button
          onClick={() => toggleInlineInput("kilometrajeActual")}
          className={`${styles.actionBtn} ${extras.kilometrajeActual ? styles.btnSuccess : styles.btnPrimary}`}
        >
          {inlineInputs.kilometrajeActual ? (
            <input
              autoFocus
              type="number"
              value={extras.kilometrajeActual}
              onChange={(e) =>
                handleExtraChange("kilometrajeActual", e.target.value)
              }
            ...
            <>
              KM ACTUAL{" "}
              {extras.kilometrajeActual ? `(${extras.kilometrajeActual})` : ""}
            </>
          )}
        </button>

        <button
          onClick={() => toggleInlineInput("proximoCambioKm")}
          className={`${styles.actionBtn} ${extras.proximoCambioKm ? styles.btnSuccess : styles.btnPrimary}`}
        >
          {inlineInputs.proximoCambioKm ? (
            <input
              autoFocus
              type="number"
              value={extras.proximoCambioKm}
              onChange={(e) =>
                handleExtraChange("proximoCambioKm", e.target.value)
              }
              onBlur={() => blurInlineInput("proximoCambioKm")}
              className="bg-transparent border-white/50 border-b outline-none w-full text-center"
              placeholder="PRÓX. CAMBIO"
            />
          ) : (
            <>
              PRÓX. CAMBIO{" "}
              {extras.proximoCambioKm ? `(${extras.proximoCambioKm})` : ""}
            </>
          )}
        </button>
        {/* FIN NUEVOS BOTONES */}
        ...
        >
          {inlineInputs.observaciones ? (
            <input
              ...
              onChange={(e) =>
                handleExtraChange("observaciones", e.target.value)
              }
            ...
            <>OBSERV. {extras.observaciones && <Check className="w-4 h-4" />}</>
          )}
        </button>
      ...
      {productosUsuales.length > 0 && cart.length === 0 && (
        <div className="flex gap-2 mb-4">
          <span className="mt-2 font-bold text-slate-500 text-xs">
            FRECUENTES:
          </span>
          {productosUsuales.map((p: any) => (
            <button
              key={p.id}
              onClick={() => addItem(p)}
              className="bg-blue-100 hover:bg-blue-600 px-3 py-1 rounded-full font-bold text-blue-700 hover:text-white text-xs transition-colors"
            >
              + {p.nombre}
            </button>
          ))}
          ...
          <table className={styles.table}>
            ...
            <tbody>
              {cart.map((item) => (
                <tr
                  key={item.tempId}
                  className="bg-white border-slate-200 last:border-0 border-b"
                >
                  ...
                  <td className="p-3 text-center">
                    <button
                      ...
                      className="flex-1 bg-red-500 hover:bg-red-600 px-3 py-2 rounded font-bold text-white text-xs transition-colors"
                    >
                      Eliminar
                    </button>
                  </td>
        ...
        <div className="flex md:flex-row flex-col justify-between items-center gap-8 pb-4">
          <div className="flex-[2] md:text-left text-center">
            ...
            <p className="mt-1 text-slate-400 text-xs">
              Subtotal: S/ {totals.subtotal.toFixed(2)} | IGV: S/{" "}
